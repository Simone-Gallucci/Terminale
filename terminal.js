...
switch (base) {
  ...
  mv(args) {
    if (args.length < 2) return this.output.push("mv: sintassi mv vecchio nuovo");
    const [oldName, newName] = args;
    const current = this.currentDir();
    if (!(oldName in current)) return this.output.push(`mv: '${oldName}': File o cartella inesistente`);
    current[newName] = current[oldName];
    delete current[oldName];
    this.output.push(`'${oldName}' rinominato in '${newName}'`);
    this.saveFS();
  },
  help() {
    const cmds = [
